...
from tasks.clean_oc_snapshot import clean_oc_snapshot_task

logger = logging.getLogger(__name__)

@celery_app.task
def rollup_minute_task(instrument_id, expiry, snapshot_time=None):
    ...
    db = SessionLocal()
    logger.info(f"[I-ROLLUP] Rolling up {instrument_id} ({expiry}). Snapshot time (UTC): {now_utc}")
    ...
    try:
        ...
        if deleted_count > 0:
            logger.info(f"[I-ROLLUP] Deleted {deleted_count} existing rows for {instrument_id} ({expiry}) at IST {ist_minute} (UTC: {now_utc})")
        ...
        if not snapshots:
            logger.warning(f"[I-ROLLUP] No snapshots found for {instrument_id} ({expiry}) at IST {ist_minute} (UTC: {now_utc})")
            db.close()
            return
        ...
        db.commit()
        logger.info(f"[I-ROLLUP] Rolled up {len(combined_rows)} rows of {instrument_id} ({expiry}) at IST {ist_minute}")

        # Trigger summary computation
        oc_summary_task.delay(instrument_id, expiry, ist_minute)

        # Trigger oc snapshot clean
        clean_oc_snapshot_task.delay(instrument_id, expiry, snapshot_time)

    except Exception as e:
        db.rollback()
        logger.error(f"[I-ROLLUP] Failed rolling up for {instrument_id} ({expiry}) at IST {ist_minute}: {e}")
    finally:
        db.close()