...
def compute_oc_summary(db, instrument_id, expiry, ist_minute):
    ...
    if not rows:
        logger.warning(f"[SUMMARY] No data found for {instrument_id} at {ist_minute}")
        return
    ...
    db.add(summary)
    db.commit()
    logger.info(f"[SUMMARY] Added summary for {instrument_id} at {ist_minute}")
